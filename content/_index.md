---
title: "Homepage"
description: "This is the homepage"
weight: ""
categories: []
tags: []
layout: "simple"
---
[ ↑ Front Matter ↑]: #
[Has to be in the first line of .md file]: #

[Divider or Horizontal Line]: # 
*** 

The purpose of this site is to discuss key concepts of hugo. Content are organized by sub-directories inside the content directory. Each will have pages that discusses a key concept about hugo.

## Table of Contents
*** 
1. Getting Started
    -  [Project Structure](/getting-started/project-structure/)
    -  [Installation](/getting-started/installation/)
    -  [Hugo Workflow](/getting-started/hugo-workflow/)

2. Concepts
    -  [Taxonomies](/concepts/taxonomies/)
    -  [Pages VS Sections](/concepts/pages-vs-sections/)
    -  [Front Matter](/concepts/front-matter/)
    -  [Content Types](/concepts/content-types/)
    -  [Archetypes](/concepts/archetypes/)

3. Examples
    -  [Short Codes](/examples/shortcodes/)
    -  [Markdown](/examples/markdown/)



[TODO] Implement tailwind css 

## Relevant Links
*** 
- [Hugo Documentation](https://gohugo.io/documentation/)
- ...

